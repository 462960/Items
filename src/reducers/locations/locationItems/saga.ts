  ...
  yield generateTypicalSaga(
    {
      ...
      partUrl: `/locations/${payload.id}/items?${toQuery(queryArgs)}`,
    },
    function*(res) {
...
function* refetchItems() {
  if (!location.pathname.match(/locations\/[^/]+\/items/i)) return;
...
function* getColumns(action: IAction) {
  ...
  yield generateTypicalSaga(
    {
      headers: getDefaultAutorizedHeaders(),
      partUrl: `/columns?route=/locations/${uuid()}/items`,
    },
    function*(res) {
